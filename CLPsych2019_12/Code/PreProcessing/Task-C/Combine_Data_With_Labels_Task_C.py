...
path ="/home/yy452/rds/rds-gvdd-Yuap0gjVpKM/yy452/umd_reddit_suicidewatch_dataset_v2/crowd/test/shared_task_posts_test.csv"
file =open(path, 'r', encoding = 'utf8')
reader_data = csv.reader(file, delimiter=',')
post_user = list(reader_data)
all_post = dict()

for i, row in enumerate(post_user):
    if(i == 0):
        continue
    all_post[row[0]] = row[1]
...
for i, row in enumerate(reader_data):
    if(i == 0):
        continue
    try:
        user_id_label[all_post[row[0]]] = all_user_id_label[row[1]]
    except:
        user_id_label[all_post[row[0]]] = ''

...
from transformers import BigBirdTokenizer as tokenizer
from transformers import BigBirdForSequenceClassification, AdamW, BigBirdConfig,get_linear_schedule_with_warmup
...
tokenizer = tokenizer.from_pretrained('google/bigbird-roberta-base', do_lower_case=True)
...
for i in range(4,5):
    ...
    data_dir = '/home/yy452/rds/rds-gvdd-Yuap0gjVpKM/yy452/CLPsych2019_12/Dataset/add_expert/emoticons_add/10_fold'
    batch_size = 8
    epochs = 75
    max_seq_length = 1024
    ...
    train_examples = processor.get_train_examples(data_dir, i+1)
    validation_examples = processor.get_dev_examples(data_dir, i+1)
    ...
    for ep in trange(int(epochs), desc="Epoch"):
        ...
        eval_examples = processor.get_dev_examples(data_dir, k)
    ...
    model_name = "BigBird_{}.bin".format(i+1)

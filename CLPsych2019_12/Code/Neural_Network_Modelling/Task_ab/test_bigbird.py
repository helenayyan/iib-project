...
# from pytorch_pretrained_bert.modeling import BigBirdForSequenceClassification, BigBirdConfig, WEIGHTS_NAME, CONFIG_NAME
# from pytorch_pretrained_bert.tokenization import BigBirdTokenizer
# from pytorch_pretrained_bert.optimization import BertAdam, WarmupLinearSchedule
CONFIG_NAME = 'long_config.json'
WEIGHTS_NAME = 'longformer_model.bin'

from transformers import BigBirdTokenizer, BigBirdForSequenceClassification, AdamW, BigBirdConfig, get_linear_schedule_with_warmup
...
def main():
    ...
    tokenizer = BigBirdTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    ...
    model = BigBirdForSequenceClassification.from_pretrained(args.bert_model,
              cache_dir=cache_dir,
    ...
    if args.do_train and args.do_eval:
        ...
        output_model_file = os.path.join(args.output_dir, WEIGHTS_NAME)
        output_config_file = os.path.join(args.output_dir, CONFIG_NAME)
        config = BigBirdConfig(output_config_file)
        model = BigBirdForSequenceClassification(config, num_labels=num_labels)
        ...
        model = BigBirdForSequenceClassification.from_pretrained(output_model_file,config=config)
        # model.load_state_dict(torch.load(output_model_file))
    model.to(device)

...
# from pytorch_pretrained_bert.modeling import LongformerForSequenceClassification, LongformerConfig, WEIGHTS_NAME, CONFIG_NAME
# from pytorch_pretrained_bert.tokenization import LongformerTokenizer
# from pytorch_pretrained_bert.optimization import BertAdam, WarmupLinearSchedule
CONFIG_NAME = 'long_config.json'
WEIGHTS_NAME = 'longformer_model.bin'

from transformers import LongformerTokenizer, LongformerForSequenceClassification, AdamW, LongformerConfig, get_linear_schedule_with_warmup
...
def main():
    ...
    tokenizer = LongformerTokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    ...
    model = LongformerForSequenceClassification.from_pretrained(args.bert_model,
              cache_dir=cache_dir,
    ...
    if args.do_train and args.do_eval:
        ...
        output_model_file = os.path.join(args.output_dir, WEIGHTS_NAME)
        output_config_file = os.path.join(args.output_dir, CONFIG_NAME)
        config = LongformerConfig(output_config_file)
        model = LongformerForSequenceClassification(config, num_labels=num_labels)
        ...
        model = LongformerForSequenceClassification.from_pretrained(output_model_file,config=config)
        # model.load_state_dict(torch.load(output_model_file))
    model.to(device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                logits = model(input_ids, token_type_ids=None, attention_mask=input_mask)[0]
        ...
        output_eval_file = os.path.join(args.output_dir, "Test_Reqd_Labels.csv")
        outcsv = open(output_eval_file,'w', encoding = 'utf8', newline='')
